...
function Frame() {
  ...
  const loadServoFunctions = async () => {
    try {
      ...
      showMessage(t('frame.errorLoading'), 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSaveModified = async () => {
    if (Object.keys(modifiedOutputs).length === 0) {
      showMessage(t('frame.noChanges'), 'info')
      return
    }
    ...
    try {
      ...
      if (errors.length === 0) {
        showMessage(t('frame.successSaveAll', { count: Object.keys(modifiedOutputs).length }), 'success')
        ...
        showMessage(t('frame.errorSavePartial', { errors: errors.join(', ') }), 'error')
      }
    } catch (error) {
      console.error('Error updating servo functions:', error)
      showMessage(t('frame.errorSave'), 'error')
    } finally {
      setSaving(false)
    }
  }
  ...
  const getFunctionName = (value) => {
    ...
    if (functionName) {
      return t(`frame.functions.${functionName}`, functionName)
    }
    return t('frame.functions.Unknown', `Unknown (${value})`)
  }

  if (!isConnected) {
    return (
      <div className="settings-section">
        <h2 className="section-title">{t('frame.title')}</h2>
        <p className="section-description">{t('frame.description')}</p>
        
        <div className="empty-state">
          <div className="empty-icon">🔌</div>
          <h3>{t('frame.noConnection')}</h3>
          <p>{t('frame.noConnectionHint')}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="settings-section">
      <div className="frame-header">
        ...
        {Object.keys(modifiedOutputs).length > 0 && (
          <button 
            className="save-modified-btn"
            onClick={handleSaveModified}
            disabled={saving}
          >
            {saving ? t('frame.saving') : `${t('frame.saveChanges')} (${Object.keys(modifiedOutputs).length})`}
          </button>
        )}
      </div>
